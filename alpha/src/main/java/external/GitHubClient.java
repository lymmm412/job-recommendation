...
public class GitHubClient {
	//why not a serverlet? because it is an internal class
	//serverlet is used to process front end request
	...
	public List<Item> search(double lat, double lon, String keyword) {
		...
		try {
			keyword = URLEncoder.encode(keyword, "UTF-8");//why encode? encode some special character that fit url format 
		...
		try {
			...
			HttpEntity entity = response.getEntity();//no result
			if (entity == null) {
				return new ArrayList<>();
			}

			BufferedReader reader = new BufferedReader(new InputStreamReader(entity.getContent()));//why buffer reader?
...
//			System.out.println("response json array: ");
